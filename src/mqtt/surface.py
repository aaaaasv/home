...
# a reading from somebody else's topic — zigbee2mqtt's, say — which the bot listens to but does not answer
ReadingHandler = Callable[[str], Awaitable[None]]
...
class MqttSurface:
    ...
    def listen_to(self, topic: str, handle: ReadingHandler) -> None:
        """
        Follow a topic that belongs to someone else, by its full name rather than a suffix of ours.
