...
from src.bot.handlers.plants.jobs import PlantComfortJob
...
class PlantComfortJobTestCase(BaseIntegrationTestCase):
    async def asyncSetUp(self):
        await super().asyncSetUp()
        # the job runs on a real household calendar, so its window is seeded against the real clock
    ...
    def build_job(self) -> PlantComfortJob:
        return PlantComfortJob(
            bot=self.bot,
            chat_id=CHAT_ID,
    ...
    async def test_plant_comfort_job_a_plant_becoming_uncomfortable_posts_a_card_and_remembers_it(self):
    ...
    async def test_plant_comfort_job_a_plant_recovering_deletes_the_card_and_posts_comfortable(self):
    ...
    async def test_plant_comfort_job_a_plant_gaining_a_problem_edits_the_card_in_place_silently(self):
        plant_id = await self.seed_plant(
            name="Кактус",
            room=ROOM,
    ...
    async def test_refresh_discomfort_cards_reposts_a_lost_card_silently(self):
        plant_id = await self.seed_plant(
            name="Кактус", room=ROOM, ideal_humidity_min_percent=45.0, ideal_humidity_max_percent=90.0
        )
        await self.seed_plant_climate_alert(
        ...
        await self.build_job().refresh_discomfort_cards()
    ...
    async def test_refresh_discomfort_cards_leaves_a_comfortable_plant_alone(self):
        await self.seed_plant(
            name="Пеперомія", room=ROOM, ideal_humidity_min_percent=30.0, ideal_humidity_max_percent=90.0
        )
